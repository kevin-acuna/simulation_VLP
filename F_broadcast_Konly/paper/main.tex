...
Recent advances in micro-electromechanical system (MEMS) micromirror gimbals~\cite{Liu:25}, liquid-crystal deflectors~\cite{Feng2019}, and optical phased arrays~\cite{mi13070990} enable high-speed redirection of a single LED beam, creating temporal diversity that replaces the spatial redundancy of multiple sources.
...
\begin{enumerate}
    ...
    \item A closed-form amplitude estimator derived from the profiled Gaussian likelihood, which, combined with nonlinear least-squares (NLS) direction finding, yields a positioning pipeline that closely tracks the $\mathrm{PEB}_\mathrm{B}$ in Monte Carlo simulations.
\end{enumerate}
...
This constitutes a broadcast positioning architecture: one LED scan serves all receivers simultaneously.
...
Unlike bounds that incorporate a dedicated ranging measurement, the $\mathrm{PEB}_\mathrm{B}$ quantifies the local lower bound on the accuracy achievable from the $K$ steered-orientation observations alone when $\mathbf{n}_r$ is known.
...
\label{eq:peb_b}
\end{equation}
This quantity lower-bounds the local root-mean-square (RMS) position error of any unbiased estimator of $\mathbf{r}$ under the observation model~\eqref{eq:obs_model}, when only the $K$ steered-orientation power measurements and known $\mathbf{n}_r$ are used.
...
Physically, receiver tilt modifies the incidence cosine and, when averaged over arbitrary tilt azimuths, generally reduces the effective received power, thereby diminishing the SNR and the magnitude of the gradient vectors.
As a consequence, the FIM~\eqref{eq:fim_broadcast} contracts and the $\mathrm{PEB}_\mathrm{B}$ increases.
This contrasts with the direction-finding stage, where normalization cancels $\eta$, making the noiseless model $\mathbf{n}_r$-independent and leaving only marginal SNR-driven tilt sensitivity~\cite{AcunaTCOM2025}.
...
In the overdetermined regime ($K\geq 4$), additional observations can improve the conditioning of~$\mathcal{I}_{\mathrm{B}}$ and reduce the bound, provided that the added orientations contribute non-redundant gradient information.
...
The orientation sets $\{\mathbf{n}_{t,i}\}_{i=1}^K$ are those optimized for the direction error bound (DEB) in~\cite{AcunaTCOM2025}; they are not re-optimized for the broadcast PEB derived here, so the results quantify the proposed distance-recovery mechanism under the same beam-steering codebooks as the reference architecture.
...
Increasing $K$ from~5 to~9 reduces $\mathrm{PEB}_\mathrm{B}$ across the floor, with the improvement most pronounced at the boundaries where the additional orientations provide supplementary gradient diversity.
...
For the considered DEB-optimized orientation sets, the broadcast bound decreases monotonically with~$K$, reflecting the additional Fisher information contributed by each orientation through a new rank-one term in the FIM~\eqref{eq:fim_broadcast}.
...
On the log--log axes, all curves exhibit a slope of $-1/2$, consistent with the theoretical $1/\sqrt{\mathrm{SNR}}$ scaling predicted by the CRLB under Gaussian noise~\cite{Kay1993}.
...
Table~\ref{tab:results} reports the 3D positioning metrics for $K\in\{5,9\}$: root-mean-square error (RMSE), 90th spatial percentile of the per-position RMSE ($P_{90}$), and average positioning error (APE).
For the $\mathrm{PEB}_\mathrm{B}$ rows, these metrics denote the corresponding spatial statistics of the local bound over the same testbed positions.
The NLS estimator achieves an RMSE of 2.49\,cm at $K=5$ and 1.73\,cm at $K=9$, closely tracking the broadcast bound ($\mathrm{PEB}_\mathrm{B}$: 2.49\,cm and 1.72\,cm, respectively).
...
The increasing GLS--WLS gap at large $K$ is consistent with the diagonal covariance approximation becoming increasingly suboptimal as the number of off-diagonal cross-correlation terms grows with~$K$.

\begin{table}[t]
\centering
\caption{Broadcast 3D positioning performance ($M{=}1{,}000$ Monte Carlo trials per position, $|\mathcal{R}|{=}1{,}792$ positions, $\mathbf{n}_r{=}[0,0,1]^{\mathsf T}$).}
\label{tab:results}
\begin{tabular}{clccc}
\toprule
$K$ & Method & RMSE [cm] & $P_{90}$ [cm] & APE [cm] \\
\midrule
...
Figure~\ref{fig:cdf} shows the spatial CDF of the per-position RMSE (computed over $M$ trials at each position) for all estimators at $K=5$ (solid lines) and $K=9$ (dashed lines).
...
All $K=9$ curves stochastically dominate their $K=5$ counterparts, in agreement with the monotonic improvement observed in the bound analysis of Section~\ref{subsec:effect_K}.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.9\linewidth]{../simulations/results/broadcast_MC1000/Fig05_CDF_broadcast}
    \caption{Spatial CDF of the per-position 3D RMSE over the testbed for $K=5$ (solid) and $K=9$ (dashed), computed with $M=1{,}000$ Monte Carlo trials per position. NLS tracks $\mathrm{PEB}_\mathrm{B}$ at both operating points; GLS and WLS exhibit structural gaps.}
    \label{fig:cdf}
\end{figure}
...
\begin{itemize}
    ...
    \item A pedestrian-carried handheld receiver ($z \approx 0.80$ m), following a random walking trajectory with mild lateral sway;
...
The parametric analysis established that: (i)~for the considered DEB-optimized orientation sets, the bound decreases monotonically with~$K$, reaching 1.72\,cm at $K=9$; (ii)~for known receiver tilt angles below $6^\circ$, the degradation remains below 5\%; and (iii)~the bound follows the theoretical $1/\sqrt{\mathrm{SNR}}$ scaling, with sub-centimeter accuracy achievable at SNR~$\geq 22$\,dB.

Monte Carlo simulations over 1,792 testbed positions showed that the NLS direction estimator, combined with the proposed broadcast distance recovery, closely tracks the $\mathrm{PEB}_\mathrm{B}$ (RMSE/$\mathrm{PEB}_\mathrm{B}\approx 1.01$), indicating that the two-stage pipeline operates near the CRLB without requiring a joint single-stage optimization.
A multi-user demonstration showed that three receivers at different heights (0.1, 0.5, and 0.8\,m)---representing a robotic vacuum, a warehouse AGV, and a pedestrian with a handheld receiver---are simultaneously localized with centimeter-level accuracy from a single $K$-orientation LED scan, without any per-user dedicated measurement or feedback.
