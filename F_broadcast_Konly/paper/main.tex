...
In this paper, we propose a broadcast beam-steered OWP architecture that achieves 3D positioning using \emph{only} the $K$ steered-orientation measurements, without any cooperative alignment or PD reorientation.
...
\begin{enumerate}
    \item A broadcast single-LED OWP architecture that achieves 3D localization from $K$ measurements alone, enabling simultaneous multi-user positioning without per-receiver dedicated measurements or feedback.
    ...
    \item A closed-form amplitude estimator derived from the profiled Gaussian likelihood, which, combined with existing NLS direction finding, yields an asymptotically efficient positioning pipeline achieving the $\mathrm{PEB}_\mathrm{B}$ within a factor of $1.01\times$.
\end{enumerate}
...
Once $\hat{\mathbf{n}}_d$ is available, the absolute power levels---discarded by the ratio-based direction estimators---are exploited to recover the amplitude parameter~$\eta$ and, subsequently, the distance~$d$.
