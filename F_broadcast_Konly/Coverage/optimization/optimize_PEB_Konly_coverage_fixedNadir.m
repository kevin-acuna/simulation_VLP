% optimize_PEB_Konly_coverage_fixedNadir.m
% =========================================================================
%  *** VARIANT: LED 1 IS PINNED AT NADIR {theta=0, rho=0} ***
% =========================================================================
% Identical to optimize_PEB_Konly_coverage.m EXCEPT that the first LED is
% forced to point straight down (nadir). This is required by the GLS method,
% which benefits from having one beam aimed at the floor. The GA then only
% optimizes the remaining K-1 beams.
%
% HOW THE PINNING WORKS (minimal & safe):
%   The nadir is imposed by COLLAPSING the bounds of the first orientation
%   (lb(1)=ub(1)=0 for theta_1, lb(2)=ub(2)=0 for rho_1). The GA keeps the
%   full 2K parametrization, so PEB_Konly_objective.m and PEB_Konly_monitor.m
%   are used UNCHANGED and still see/plot all K beams (including the nadir).
%   rho_1 is irrelevant at theta_1=0 (n_t=[0,0,-1]) but is pinned for cleanliness.
%
% WHY A SEPARATE FILE: the nadir pin is a deliberate design choice for GLS,
%   NOT the default. Keeping it in its own clearly-named script prevents it
%   from silently affecting a normal (free) optimization run.
%
% Results are saved under results/PEB_Konly_coverage_fixedNadir/ so they do
% NOT mix with the free-optimization results.
%
...
%% ======================== CONFIGURATION ========================
K_orientations = 9;             % GLS focus: K=9 with LED 1 pinned at nadir.
                                % (add more, e.g. [5,9], if you want.)
...
results_dir = fullfile(this_dir, 'results', 'PEB_Konly_coverage_fixedNadir');
% ===============================================================
...
for k_idx = 1:length(K_orientations)
    ...
    fprintf('*** LED 1 PINNED AT NADIR [0,0] (GLS variant) ***\n');
    fprintf('%s\n', repmat('=', 1, 60));
    fprintf('Date: %s\n', datestr(now));
    fprintf('Metric: %s\n', system_params.optimization_metric);
    fprintf('K = %d orientations (1 fixed nadir + %d free)\n', K, K-1);
    ...
    % --- PIN LED 1 AT NADIR {theta_1=0, rho_1=0} for the GLS method ---
    % Collapsing the bounds (lb==ub) forces the GA to keep this beam fixed;
    % it effectively searches only the remaining K-1 beams. rho_1 is
    % irrelevant at theta_1=0 (n_t=[0,0,-1]) but is pinned for cleanliness.
    lb(1) = 0; ub(1) = 0;   % theta_1 = 0 deg  (nadir)
    lb(2) = 0; ub(2) = 0;   % rho_1   = 0 deg
    ...
    %% Run optimization
    fprintf('Starting GA optimization for PEB_B (K=%d, LED 1 fixed nadir)...\n', K);
    t_start = tic;
    ...
    %% Process results
    fprintf('\n%s\n', repmat('-', 1, 50));
    fprintf('PEB_B OPTIMIZATION RESULTS FOR K = %d (LED 1 fixed nadir)\n', K);
    fprintf('%s\n', repmat('-', 1, 50));
    ...
    fprintf('\nOptimal LED orientations (PEB_B-optimized, LED 1 = nadir):\n');
    for i = 1:K
        ...
        fprintf('  LED %d: theta = %6.2f deg, rho = %6.2f deg -> n_t = [%6.3f, %6.3f, %6.3f]%s\n', ...
            i, theta_deg, rho_deg, nt(1), nt(2), nt(3), tag);
    end
    ...
    fprintf('orientations_fixedNadir_K%d = [', K);
    for i = 1:length(xOpt)
        if i < length(xOpt)
            fprintf('%.2f,', xOpt(i));
        else
