...
for i_pos = 1:N_pos
    ...
    for i_dir = 1:size(n_t,1)
        ...
        P_r_raw = (P_r{i_pos,i_dir} + sqrt(sigma2).*randn(1,1000))./(-C); % Noise power observed after normalization (needed for the non-linear MATLAB solver to coverge) [W]
        P_r_noisy{i_pos,i_dir} = min(max(P_r_raw, 0.00000000001), 1000);
    end
end
