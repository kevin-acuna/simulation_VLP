...
sigma2 = 30e6*10^(-21.0)/(R_pd^2); % AWGN variance [A²]
...
%         P_r_noisy{i_pos,i_dir} = (P_r{i_pos,i_dir} + sqrt(sigma2).*all_noise(i_pos,i_dir,:)); % Noise power observed after normalization (needed for the non-linear MATLAB solver to coverge) [W]
        
        % Calculo de SNR de referencia
...
% 2. Reemplazar -Inf por -80 en esas posiciones:
...
for i_pos = 1:N_pos
    ...
    P_r_axis_noisy_GLS = P_r_axis_GLS + sqrt(sigma2).*randn(1,1000); % Potencia observada con ruido
...
errorNormSVD = errorNormSVD(~isnan(errorNormSVD));
...
for i = 1:N_pos
    ...
    % sigma2_crlb = sigma2/R_pd^2;       % Varianza del ruido (W^2)
    sigma2_crlb = sigma2/1; % 1/N
    % sigma2_crlb = sigma2_crlb/1000; %1000 muestras

    % Calcular CRLB para esta posición
    % peb_value = vlp_peb(R_real, n_t(1:N_or,:)', T', m_t, K, sigma2_crlb);
    peb_value = vlp_peb_beam(R_real, n_t(1:N_or,:)', T', m_t, K, sigma2, 1000, 1000);
    % Verificar si el valor es imaginario
    if ~isreal(peb_value)
        valid_indices(i) = false;  % Marcar como no válido
        num_imaginarios = num_imaginarios + 1;
        fprintf('Punto %d: valor CRLB imaginario detectado y filtrado.\n', i);
    else
...
legend('Linear estimator of P_{r,i} + SVD', 'WLS(SNR)', 'WLS', 'GLS', 'non-linear','CRLB (theoretical bound)','Location', 'southeast');
