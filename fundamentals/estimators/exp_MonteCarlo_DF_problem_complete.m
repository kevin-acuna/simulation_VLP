...
% spatial Root Mean Square Error (RMSE). It compares them against the 
% theoretical DEB (Direction Error Bound).
%
% Author: Kevin Acuña
...
if strcmp(receiver_mode, 'fixed')
    ...
    N_pos = 100; % Reduced for random testing
...
fprintf('This may take some time depending on TEST_MODE.\n');
...
for i_pos = 1:N_pos
    ...
    % Calculate theoretical DEB for this position
    ...
    % Arrays to store errors for the M_trials
    err_WLS_mc = zeros(M_trials, 1);
    err_GLS_mc = zeros(M_trials, 1);
    err_NL_mc  = zeros(M_trials, 1);
    
    for mc = 1:M_trials
        % Generate noise (Hardware sampling equivalent)
        % Fast generation of N_samples x N_or noisy matrix
        ...
        err_WLS_mc(mc) = norm(v_true - v_est_wls') * rad2deg_factor;
        ...
        err_GLS_mc(mc) = norm(v_true - v_est_gls') * rad2deg_factor;
        ...
        err_NL_mc(mc) = norm(v_true - v_est_nl') * rad2deg_factor;
        time_NL = time_NL + toc;
    end
    
    % Calculate empirical spatial RMSE for this position
...
[f, x] = ecdf(rmse_ang_GLS_pos); 
stairs(x, f, '-', 'LineWidth', 1.5, 'Color', color_gls);

[f, x] = ecdf(rmse_ang_WLS_pos); 
stairs(x, f, '-', 'LineWidth', 1.5, 'Color', color_wls);

[f, x] = ecdf(rmse_ang_NL_pos); 
stairs(x, f, '-', 'LineWidth', 1.5, 'Color', color_nl);

[f, x] = ecdf(rmse_ang_DEB_pos(~isnan(rmse_ang_DEB_pos))); 
