...
%
% Modified for K=9 orientations - B. Béchadergue - LISV - May 2025
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
close all;
clear variables;
clc;
...
n_t = zeros(N_or, 3);
% Using fixed optimized Tx orientation for K=9
for i = 1:N_or
...
for i_pos = range
    ...
    % 2. Definition of the polynomials Q_i and L for K=9 orientations
...
title('Performance Comparison - K=9 Orientations');
grid on;

%% Display Statistics for K=9
fprintf('\n=== PERFORMANCE STATISTICS FOR K=9 ORIENTATIONS ===\n');
fprintf('Number of receiver positions evaluated: %d\n', length(range));
fprintf('Mean RMS error: %.3f cm\n', mean(errorNorm)*100);
fprintf('Median RMS error: %.3f cm\n', median(errorNorm)*100);
fprintf('Standard deviation: %.3f cm\n', std(errorNorm)*100);
fprintf('Min RMS error: %.3f cm\n', min(errorNorm)*100);
fprintf('Max RMS error: %.3f cm\n', max(errorNorm)*100);
fprintf('Average execution time per position: %.4f s\n', mean(time_NL));
fprintf('Total simulation time: %.2f s\n', sum(time_NL));

% 90th percentile analysis
percentile_90 = prctile(errorNorm*100, 90);
fprintf('90th percentile RMS error: %.3f cm\n', percentile_90);

% Percentage of positions with error < 5cm
positions_under_5cm = sum(errorNorm*100 < 5) / length(errorNorm) * 100;
fprintf('Percentage of positions with error < 5cm: %.1f%%\n', positions_under_5cm);

fprintf('\nOrientation vectors used (K=9):\n');
for i = 1:N_or
    theta_deg = all_orientations{7}(2*i-1);
    rho_deg = all_orientations{7}(2*i);
    fprintf('  Orientation %d: θ=%.2f°, ρ=%.2f° -> n_t=[%.3f, %.3f, %.3f]\n', ...
        i, theta_deg, rho_deg, n_t(i,1), n_t(i,2), n_t(i,3));
end
