...
orientations_K4 = [25.0, 0.0, 25.0, 90.0, 25.0, 180.0, 25.0, 270.0];
orientations_K5 = [25.0, 0.0, 25.0, 72.0, 25.0, 144.0, 25.0, 216.0, 25.0, 288.0];

% Store all orientation sets in a cell array
all_orientations = {orientations_K3, orientations_K4, orientations_K5};
K_values = [3, 4, 5];
...
for j = 1:length(K_values)
    ...
    plot(sigma2_values, peb_results(:, j), markers{j}, 'Color', colors{j}, ...
...
xlabel('Noise Variance \sigma^2 (W²)', 'FontSize', 12);
ylabel('Position Error Bound (m)', 'FontSize', 12);
title('Position Error Bound vs. Noise Level for Different Orientation Sets', 'FontSize', 14);
grid minor;
...
% ax = gca;
% ax.XTickMode = 'auto';
% % Format the x-axis tick labels as scientific notation
% set(ax, 'XTickLabelMode', 'auto');
% 
% % Set y-axis limits based on all data
% y_min = min(peb_results(:)) * 0.8;
% y_max = max(peb_results(:)) * 1.2;
% ylim([y_min, y_max]);
