function [cost] = Complex_Cost_Orientation(n_t_s)
...
n_t = [n_t_1;n_t_2;n_t_3];
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%               Incorporating Average SNR Metric                  %%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Compute SNR (in dB) at each receiver point for each transmitter.
    SNR_total = zeros(N_rx, N_ry, N_n_t);
    for i_n = 1:N_n_t
        % SNR in dB: 10*log10((R_pd*P_r_real)^2/sigma2_tot)
        SNR_total(:,:,i_n) = 10*log10((R_pd * P_r_real(:,:,i_n)).^2 / sigma2_tot);
    end
    avgSNR = mean(SNR_total(:));  % Average SNR in dB
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%           Combine RMS and SNR into a Single Cost Function         %%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Choose normalization factors based on typical ranges.
    % (Adjust RMS_norm and SNR_norm as needed.)
    RMS_norm = 0.12;    % For example, if RMS is typically around 1 meter.
    SNR_norm = 15.0;   % For example, if SNR is typically around 30 dB.
    
    % Choose weights (the relative importance of RMS and SNR).
    w_RMS = 2.0;
    w_SNR = 1.0;
    
    % Since we want to minimize RMS and maximize SNR, we subtract the normalized SNR.
    % Higher SNR reduces the overall cost.
    combined_cost = w_RMS * (cdf90_RMS / RMS_norm) - w_SNR * (avgSNR / SNR_norm);
    
    % Return the combined cost as the output.
    cost = combined_cost;
