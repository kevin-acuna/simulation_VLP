...
import { Activity, Circle, Grid2X2, Rows3, Square, Box, Lightbulb, Radio, ScanEye, X, RotateCcw, Check, Download } from 'lucide-react'
...
export type PanelTab = 'room' | 'lighting' | 'receiver' | 'optics' | 'view'
  ...
  return (
    <aside id="scene-settings" className="control-panel" aria-label="Scene configuration">
      ...
      <div className="panel-tabs" role="tablist" aria-label="Configuration sections">
        {([{ id: 'room', icon: Box, label: 'Room' }, { id: 'lighting', icon: Lightbulb, label: 'Lighting' }, { id: 'receiver', icon: Radio, label: 'Receiver' }, { id: 'optics', icon: Activity, label: 'Optics' }, { id: 'view', icon: ScanEye, label: 'View' }] as const).map(({ id, icon: Icon, label }) => (
